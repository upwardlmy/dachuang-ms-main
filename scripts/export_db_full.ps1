...
# Defaults align with backend/config/settings.py (and scripts/export_db_full.sh)
...
$outputFile = if ($env:OUTPUT_FILE) { $env:OUTPUT_FILE } else { (Join-Path $repoRoot "dachuang_db_full.sql") }
...
Write-Host "Exported database to $outputFile"
